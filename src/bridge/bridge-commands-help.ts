...
export function formatBridgeCommandsHelp(_backend: AcpBackend): string {
  ...
  return [
    ...
    "• `/new <序号> --backend <cursor-official|cursor-legacy|claude|codex|codex-app-server>` — 用列表第 N 项创建并切换到新 session；也支持 `-b <backend>`，如 `-b cc`",
    "• `/new <目录绝对路径> --backend <cursor-official|cursor-legacy|claude|codex|codex-app-server>` — 指定工作区；可附 `--name <名称>`，`-b cc` 等同 `--backend claude`",
