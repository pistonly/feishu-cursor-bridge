...
export function formatBridgeCommandsHelp(backend: AcpBackend): string {
  ...
  return [
    ...
    "• `/new <序号> --backend <cursor-official|cursor-legacy|claude|codex>` — 用列表第 N 项创建并切换到新 session",
    "• `/new <目录绝对路径> --backend <cursor-official|cursor-legacy|claude|codex>` — 指定工作区；可附 `--name <名称>`",
