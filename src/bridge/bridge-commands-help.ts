...
export function formatBridgeCommandsHelp(_backend: AcpBackend): string {
  ...
  return [
    ...
    "若设 `BRIDGE_GROUP_SESSION_SCOPE=shared`，群内共享同一组 session；此时 `/new`、`/switch`、`/close`、`/resume`、`/mode`、`/model`、`/compact`、`/stop` 等管理命令仅管理员可用。",
    "",
    acpNote,
    "",
    "**Session 与工作区**",
    ...
    "• `/compact` — 对 `codex-app-server` 当前 session 调用原生上下文压缩；其它 backend 不由桥接拦截",
    "• `/stop` / `/cancel` — 若当前槽位有排队消息则先撤销排队；否则中断正在生成的回复",
