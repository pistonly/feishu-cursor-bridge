...
- **适配器脚本路径**（仅 `ACP_BACKEND=cursor-legacy`）：固定相对**桥接仓库根目录**解析（`src/config.ts` / `src/acp/paths.ts`），规则与桥接自身一致——主进程为 **`dist/index.js`** → 适配器 **`cursor-agent-acp/dist/bin/cursor-agent-acp.js`**；主进程为 **`tsx …/src/index.ts`**（`npm run dev`）→ **`cursor-agent-acp/src/bin/cursor-agent-acp.ts`** + **tsx**。克隆后需 `npm install` 以生成生产用 `dist/`。
