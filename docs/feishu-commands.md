...
**作用**：管理**当前活跃 session（当前槽位）**的排队消息或正在进行中的模型回复。若该槽位存在排队消息，优先只撤销排队消息；若没有排队消息但正在生成，桥接才会对该 ACP session 调用 SDK `cancel`（即 **`session/cancel` 通知**）。**不会**关闭 session，之后可照常发消息继续对话。
...
- `/stop` / `/cancel` 在有排队消息时会**只撤销当前槽位的排队消息**，不会中断正在生成的回复；如果仍需中断当前生成，请在队列清空后再次发送 `/stop`
...
**作用**：由 bridge 直接在后台执行 `bash service.sh upgrade`，按既有升级链路完成：

- `git fetch`
- 对目标分支执行 `git pull --ff-only`
- `npm install`
- `npm run build`
- 重启 bridge 服务
...
**`--force`**：若当前仍有飞书请求正在处理中，默认会拒绝执行升级；此时可显式使用 `/upgrade --force` 跳过该检查。

**结果持久化**：最近一次升级尝试会写入 `BRIDGE_UPGRADE_RESULT_FILE`（默认 `~/.feishu-cursor-bridge/upgrade-result.json`），因此 bridge 重启后，`/status` 仍可查看最近升级结果。
