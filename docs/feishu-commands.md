...
**作用**：绕过当前 slot 的本地“上一个请求还在处理中”保护，直接调用 ACP `prompt`，用于排查究竟是桥接层拦住了，还是上游 ACP 其实仍然能继续响应。
...
- 这是**实验命令**，不会走桥接层正常的同 slot 串行保护。
- 若同一 session 里原本就有请求在跑，新的 `session/update` 与结果**可能交错**，因此只建议在排障时临时使用。
- 命令会先回一条“已开始实验”的提示；若 ACP 最终返回，桥接会再补一张结果卡片，展示 `stopReason` 与本次期间捕获到的 `session/update` 文本。
