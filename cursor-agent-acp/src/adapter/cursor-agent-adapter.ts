...
export class CursorAgentAdapter implements ClientConnection {
  ...
  private async handleSessionNew(request: AnyRequest): Promise<{
    ...
    // Create cursor-agent chat for this session (or reuse id from client _meta / metadata)
    const metaCursorChatId =
      typeof params._meta?.cursorChatId === 'string' &&
      params._meta.cursorChatId.length > 0
        ? params._meta.cursorChatId
        : undefined;
    let cursorChatId: string | undefined =
      typeof metadata['cursorChatId'] === 'string' &&
      metadata['cursorChatId'].length > 0
